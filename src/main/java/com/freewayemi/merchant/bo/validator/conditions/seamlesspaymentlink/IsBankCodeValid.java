...
@Component
public class IsBankCodeValid implements Conditions {

    private static final Logger LOGGER = LoggerFactory.getLogger(IsBankCodeValid.class);

    @Autowired
    public IsBankCodeValid() {
    }
    ...
    public void executeConditions(Object obj) throws FreewayException {
        ...
        if (Objects.isNull(BankEnum.getCode(request.getCardInfo().getCode()))) {
            LOGGER.error("Bank code: {} does not exist for pg transaction request", request.getCardInfo().getCode());
            throw new FreewayCustomException(TransactionCode.FAILED_158);
        }
    }
